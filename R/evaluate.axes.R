...
setMethod('evaluate.axes', "Algorithm.SDM", function(obj, cv, cv.param, thresh = 1001,
  ...
  if (axes.metric == "Pearson") {
    if(obj@name == "MAXNET.SDM")
      o.predicted.values <- predict(get_model(obj, ...), obj@data[,4:length(obj@data)], type = "logistic")  # original model predicted values
    else
      o.predicted.values <- predict(get_model(obj, ...), obj@data[,4:length(obj@data)])  # original model predicted values
  }
  ...
  if(dim(obj@variable.importance)[2] == 1)
    ...
    for (i in 4:length(obj@data)) {
      ...
      if (axes.metric != "Pearson") {
        ...
        if(obj@name == "MAXNET.SDM")
          predicted.values <- predict(model.axes, obj.axes@data[,4:length(obj.axes@data)], type = "logistic")
        else
          predicted.values <- predict(model.axes, obj.axes@data[,4:length(obj.axes@data)])
